...
// CacheValueContainer_string_strings is a wrapper for strings cached data
type CacheValueContainer_string_strings struct {
	val    []string
	recent bool
}

// CacheContainer_string_strings is a wrapper for a cache keyed on KEY
type CacheContainer_string_strings struct {
	Lock          sync.RWMutex
	Cache         map[string]*CacheValueContainer_string_strings
...
// NewCache_string_strings creates a new cache, with a cache sweep
// performed at SweepInterval to look for unused entries.  Any
// cache entry not used since the last sweep, is purged.
func NewCache_string_strings(StatsName string, MaxSize int, SweepInterval time.Duration) *CacheContainer_string_strings {
	c := new(CacheContainer_string_strings)
...
func (c *CacheContainer_string_strings) Get(k string) (v []string, ok bool) {
...
// Set (k string, v []string) stores
// a value into the cache, and marks the value as "recent" so it survives
// at least one round of cache maintenance.
func (c *CacheContainer_string_strings) Set(k string, v []string) {
	saved := false
	c.Lock.Lock() // Read+Write Lock
	if len(c.Cache) < c.MaxSize {
		n := new(CacheValueContainer_string_strings)
		n.val = v
		n.recent = true
		c.Cache[k] = n
		saved = true
...
func (c *CacheContainer_string_strings) CheckConfig() {
...
func (c *CacheContainer_string_strings) ClearCache() {
	c.CheckConfig() // Get latest values for sleeping, max size
	c.Lock.Lock()   // Read+Write Lock
	c.Cache = make(map[string]*CacheValueContainer_string_strings, 16384)
	c.Lock.Unlock()
}
...
func (c *CacheContainer_string_strings) CleanCache() {
	...
	c.Lock.Unlock()
	Debugf("Cleaned cache string strings time=%s keys: %v -> %v\n", since.String(), startSize, stopSize)
...
func (c *CacheContainer_string_strings) maintenance() {
	for {
...
func (c *CacheContainer_string_strings) SetMaxSize(MaxSize int) {
	c.Lock.Lock() // Read+Write Lock
	c.MaxSize = MaxSize
	c.Lock.Unlock()
}
func (c *CacheContainer_string_strings) GetMaxSize() (MaxSize int) {
	c.Lock.Lock() // Read+Write Lock
	MaxSize = c.MaxSize
	c.Lock.Unlock()
	return MaxSize
...
func (c *CacheContainer_string_strings) SetInterval(t time.Duration) {
	c.Lock.Lock() // Read+Write Lock
	c.SweepInterval = t
	c.Lock.Unlock()
}
func (c *CacheContainer_string_strings) GetInterval() time.Duration {
	c.Lock.Lock() // Read+Write Lock
	t := c.SweepInterval
	c.Lock.Unlock()
	return t
