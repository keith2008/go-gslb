...
// CacheValueContainer_QueryInfo_MsgCacheRecord is a wrapper for MsgCacheRecord cached data
type CacheValueContainer_QueryInfo_MsgCacheRecord struct {
	val    MsgCacheRecord
	recent bool
}

// CacheContainer_QueryInfo_MsgCacheRecord is a wrapper for a cache keyed on KEY
type CacheContainer_QueryInfo_MsgCacheRecord struct {
	Lock          sync.RWMutex
	Cache         map[QueryInfo]*CacheValueContainer_QueryInfo_MsgCacheRecord
	SweepInterval time.Duration
}

// NewCache_QueryInfo_MsgCacheRecord creates a new cache, with a cache sweep
// performed at SweepInterval to look for unused entries.  Any
// cache entry not used since the last sweep, is purged.
func NewCache_QueryInfo_MsgCacheRecord(SweepInterval time.Duration) *CacheContainer_QueryInfo_MsgCacheRecord {
	c := new(CacheContainer_QueryInfo_MsgCacheRecord)
...
func (c *CacheContainer_QueryInfo_MsgCacheRecord) Get(k QueryInfo) (v MsgCacheRecord, ok bool) {
...
func (c *CacheContainer_QueryInfo_MsgCacheRecord) Set(k QueryInfo, v MsgCacheRecord) {
	c.Lock.Lock() // Read+Write Lock
	n := new(CacheValueContainer_QueryInfo_MsgCacheRecord)
	n.val = v
	n.recent = true
	c.Cache[k] = n
	c.Lock.Unlock()
...
func (c *CacheContainer_QueryInfo_MsgCacheRecord) ClearCache() {
	c.Lock.Lock() // Read+Write Lock
	c.Cache = make(map[QueryInfo]*CacheValueContainer_QueryInfo_MsgCacheRecord, 16384)
	c.Lock.Unlock()
}
...
func (c *CacheContainer_QueryInfo_MsgCacheRecord) CleanCache() {
...
func (c *CacheContainer_QueryInfo_MsgCacheRecord) maintenance() {
	for {
		...
		Debugf("Cleaned cache QueryInfo MsgCacheRecord time=%s keys: %v -> %v\n", since.String(), startSize, stopSize)
