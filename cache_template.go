//+build generate
//go:generate -command doit command ./mygen.pl cache_template.go
//go:generate doit templated_cache_string_string.go            KEYTYPE=string  VALTYPE=string VALNAME=string VALDEFAULT=""
//go:generate doit templated_cache_string_strings.go           KEYTYPE=string  VALTYPE=[]string VALNAME=strings VALDEFAULT=nil
//go:generate doit templated_cache_LookupBEKey_strings.go      KEYTYPE=LookupBEKey VALTYPE=[]string VALNAME=strings  VALDEFAULT=[]string{}
...
// CacheValueContainer_KEYTYPE_VALNAME is a wrapper for VALNAME cached data
type CacheValueContainer_KEYTYPE_VALNAME struct {
	val    VALTYPE
	recent bool
}

// CacheContainer_KEYTYPE_VALNAME is a wrapper for a cache keyed on KEY
type CacheContainer_KEYTYPE_VALNAME struct {
	Lock          sync.RWMutex
	Cache         map[KEYTYPE]*CacheValueContainer_KEYTYPE_VALNAME
...
// NewCache_KEYTYPE_VALNAME creates a new cache, with a cache sweep
// performed at SweepInterval to look for unused entries.  Any
// cache entry not used since the last sweep, is purged.
func NewCache_KEYTYPE_VALNAME(StatsName string, MaxSize int, SweepInterval time.Duration) *CacheContainer_KEYTYPE_VALNAME {
	c := new(CacheContainer_KEYTYPE_VALNAME)
...
// Get (k KEYTYPE) checks the cache
// for an existing element. returns ok=true on success, with the cached value.
func (c *CacheContainer_KEYTYPE_VALNAME) Get(k KEYTYPE) (v VALTYPE, ok bool) {
	...
	return VALDEFAULT, false
}

// Set (k KEYTYPE, v VALTYPE) stores
// a value into the cache, and marks the value as "recent" so it survives
// at least one round of cache maintenance.
func (c *CacheContainer_KEYTYPE_VALNAME) Set(k KEYTYPE, v VALTYPE) {
	saved := false
	c.Lock.Lock() // Read+Write Lock
	if len(c.Cache) < c.MaxSize {
		n := new(CacheValueContainer_KEYTYPE_VALNAME)
		n.val = v
		n.recent = true
		c.Cache[k] = n
		saved = true
...
func (c *CacheContainer_KEYTYPE_VALNAME) CheckConfig() {
...
func (c *CacheContainer_KEYTYPE_VALNAME) ClearCache() {
	c.CheckConfig() // Get latest values for sleeping, max size
	c.Lock.Lock()   // Read+Write Lock
	c.Cache = make(map[KEYTYPE]*CacheValueContainer_KEYTYPE_VALNAME, 16384)
	c.Lock.Unlock()
}
...
func (c *CacheContainer_KEYTYPE_VALNAME) CleanCache() {
	...
	c.Lock.Unlock()
	Debugf("Cleaned cache KEYTYPE VALNAME time=%s keys: %v -> %v\n", since.String(), startSize, stopSize)
...
func (c *CacheContainer_KEYTYPE_VALNAME) maintenance() {
	for {
