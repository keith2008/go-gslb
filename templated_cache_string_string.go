...
// CacheValueContainer_string_string is a wrapper for string cached data
type CacheValueContainer_string_string struct {
	val    string
	recent bool
}

// CacheContainer_string_string is a wrapper for a cache keyed on KEY
type CacheContainer_string_string struct {
	Lock          sync.RWMutex
...
func (c *CacheContainer_string_string) Get(k string) (v string, ok bool) {
...
func (c *CacheContainer_string_string) CleanCache() {
	...
	c.Lock.Unlock()
	Debugf("Cleaned cache string string time=%s keys: %v -> %v\n", since.String(), startSize, stopSize)
...
func (c *CacheContainer_string_string) maintenance() {
	for {
